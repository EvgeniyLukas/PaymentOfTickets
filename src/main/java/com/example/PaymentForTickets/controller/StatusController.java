...
//    }
        ...
        while (true) {
            try {
                for (Application application : all) {
                    if (application.getStatus() != null && application.getStatus().equals(Status.PROCESSED)) {
                        Long id = application.getId();
                        logger.debug("Application id = {}", id);
                        return webClient
                                .put()
                                .uri("new/{id}", id)
                                .retrieve()
    ...
    @PutMapping("new/{id}")
    public Status setStatus(@PathVariable("id") Application application) {
        ...
        logger.debug("Status random {}", one.getStatus());
        return repository.save(one).getStatus();
    }
